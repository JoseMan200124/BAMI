...
function generateOpsData(seed = Date.now()) {
    const r = rand(seed)
    const totalCases = 120 + Math.floor(r() * 90) // 120-210
    const aprobados = Math.floor(totalCases * (0.55 + r() * 0.2)) // 55-75%
    const alternativas = Math.floor(totalCases * (0.07 + r() * 0.06)) // 7-13%
    const enRevision = Math.floor(totalCases * (0.12 + r() * 0.08)) // 12-20%
    ...
    const byProduct = {
        'Tarjeta de Crédito': Math.floor(totalCases * (0.38 + r() * 0.1)),
        'Préstamo Personal': Math.floor(totalCases * (0.24 + r() * 0.12)),
        'Cuenta de Ahorro': Math.floor(totalCases * (0.16 + r() * 0.1)),
        'Seguro de Vida': totalCases, // placeholder
    }
    // ajustar suma
    const currentSum = byProduct['Tarjeta de Crédito'] + byProduct['Préstamo Personal'] + byProduct['Cuenta de Ahorro']
    byProduct['Seguro de Vida'] = Math.max(0, totalCases - currentSum)
    ...
    const sla = { avg_minutes: 25 + Math.floor(r() * 55) } // 25-80
...
export default function BamOpsPanel() {
    ...
    const [loadedOnce, setLoadedOnce] = useState(false) // asegura render una vez

    // Cargar una vez (intenta backend, si falla usa sim)
    useEffect(() => {
        const load = async () => {
            try {
    ...
    // Escuchar simulación SOLO una vez cuando el Autopilot lo solicite
    useEffect(() => {
        const onSim = (e) => {
            if (loadedOnce) return // ya tenemos una foto
