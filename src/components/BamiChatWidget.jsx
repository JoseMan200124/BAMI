...
const PRODUCT_SYNONYMS = {
    'Tarjeta de Crédito': ['tarjeta de credito','tarjeta de crédito','tarjeta','tc','visa','mastercard','master card','credito de tarjeta'],
...
export default function BamiChatWidget({ variant='floating', disableFloatingTrigger=false }) {
    ...
    // SSE narración IA (si hay expediente)
    useEffect(()=>{
    ...
    const askMissing=async()=>{
        ...
        if(!current) return push('bami','Aún no hay expediente. Escribe **“nuevo”** o dime el producto.')
    ...
    const openUploadManual=()=>{ if(!getCase()) return push('bami','Primero creo tu expediente. Dime el producto.'); push('user','Subir documentos'); setShowUpload(true) }
    const afterUpload=async()=>{ push('bami','📥 Recibí tus archivos. Voy leyendo lo que enviaste…'); await refreshTracker(); push('bami','Si quieres, puedo **validar con IA** para darte un resultado preliminar.'); window.dispatchEvent(new Event('ui:tracker:open')) }
    const validateAI=async(silent=false)=>{
        ...
        try{ const {message}=await validateWithAI(); setTyping(false); push('ai',`Resultado: ${message}`); await refreshTracker(); window.dispatchEvent(new Event('ui:tracker:open')) }
    ...
    const connectAdvisor=()=>{ setMode('advisor'); push('user','Hablar con un asesor'); setTyping(true); setAdvisorConnected(false); setAdvisorQueue(3); const it=setInterval(()=>setAdvisorQueue(q=>q>1?q-1:q),800); setTimeout(()=>{ clearInterval(it); setTyping(false); setAdvisorConnected(true); push('advisor','¡Hola! Soy Sofía, tu asesora. ¿Me confirmas tu DPI para verificar el expediente?'); window.dispatchEvent(new Event('ui:tracker:open')) },2400) }
    ...
    const handleSend=async(e)=>{
        ...
        if(action==='create_case' && product) return newCase(product)
        if(action==='create_case' && !product){ push('bami','Perfecto, ¿para qué producto deseas aplicar? Elige una opción o escríbela:'); push('bami','• Tarjeta de Crédito\n• Préstamo Personal\n• Hipoteca\n• PyME'); return }
        if(action==='ask_requirements'){ const p=product || (getCase()?.product ?? 'Tarjeta de Crédito'); const req=PRODUCT_RULES[p]?.map(r=>r.replaceAll('_',' ')).join(', ') || 'requisitos básicos'; push('bami',`Para **${p}** se requiere: ${req}. ¿Deseas que abra tu expediente ahora?`); return }
        if(action==='ask_times'){ push('bami','El tiempo típico de análisis es **8–24h hábiles**. Puedo iniciar tu expediente cuando me indiques el producto.'); return }
    ...
    const HeaderTab=({id,icon:Icon,children})=>(
        <button onClick={()=>setMode(id)} className={`flex items-center gap-2 px-3 py-1.5 rounded-xl text-sm ${mode===id?'bg-white shadow':'hover:bg-white/60'}`}>
        ...
        <div className="flex flex-col">
            <div className="flex items-center justify-between px-3 py-2 bg-bami-yellow/60">
                ...
                <div className="flex items-center gap-2 text-xs">
                    ...
                    <button className="ml-1 sm:ml-2 border rounded-md px-2 py-1 inline-flex items-center gap-1"
                    ...
                    <button className="border rounded-md px-2 py-1 inline-flex items-center gap-1"
                            ...
                            title="Panel Ops" aria-label="Abrir panel de análisis">
    ...
    const ChatBody=(
        <div ref={listRef} className="flex-1 overflow-auto p-3 space-y-3 bg-white" aria-live="polite" aria-label="Mensajes del chat">
    ...
    const QuickActions=(
        <div className="px-2 pt-2">
            <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2 snap-x">
                ...
                {/* Ahora abre el Tracker en overlay, no navega */}
                <button type="button"
                        onClick={()=>window.dispatchEvent(new Event('ui:tracker:open'))}
                        className="px-3 py-1.5 text-xs rounded-full border bg-white whitespace-nowrap hover:bg-gray-50 snap-start">
                    Tracker
                </button>
            </div>
    ...
    const ChatInputRow=(
        <div className="p-2 grid grid-cols-1 sm:grid-cols-12 gap-2">
            <div className="sm:col-span-9 flex gap-2">
                <input ref={inputRef} className="flex-1 border rounded-xl px-3 h-12 text-sm"
                       placeholder='Dime: “aplicar a tarjeta de crédito”, “requisitos de hipoteca”, “subir documentos”…'
                       aria-label="Mensaje para BAMI" inputMode="text"/>
            </div>
            <div className="sm:col-span-3 flex">
                <button className="btn btn-dark text-xs px-3 h-12 w-full sm:min-w-[100px] rounded-xl inline-flex items-center justify-center gap-1"
                        onClick={handleSend} aria-label="Enviar mensaje">
                    <Send size={14}/> Enviar
                </button>
            </div>
        </div>
    ...
    const ChatFooter=(<div className="border-t bg-gray-50 sticky bottom-0">{QuickActions}<form onSubmit={handleSend} className="border-t">{ChatInputRow}</form></div>)

    const containerStyle = variant==='fullscreen'
        ? { height: 'calc((var(--vh, 1vh) * 100) - 160px)' }
        : (variant==='panel' ? { height: 520 } : { height: 520 })

    const ChatWindow=(<div className="flex flex-col" style={containerStyle}>{ChatHeader}{ChatBody}{ChatFooter}</div>)
    ...
    if(disableFloating){
        // No renderizar botón ni ventana flotante
        return (<UploadAssistant open={showUpload} onClose={()=>setShowUpload(false)} onUploaded={afterUpload}/>)
    }

    // Si no está deshabilitado, se comporta como antes
    return (
        <>
            <button
                onClick={()=>setOpen(v=>!v)}
